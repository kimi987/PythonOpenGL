...
class Player:
    ...
    def get_up(self):
        forwards = self.get_forward()
        right = np.cross(a = forwards, b=self.global_up)

        return np.cross(a=right, b=forwards)
        pass
...
class Scene:
    ...
    def __init__(self):
        self.cubes = [
            Cube(
                position = [np.random.uniform(-10, 10) for x in range(3)],
                eulers = [np.random.uniform(0, 360) for x in range(3)],
                eulerVelocity = [np.random.uniform(-0.1, 0.1) for x in range(3)]
            )
        ...
        self.lights = [
            Light(
                position = [np.random.uniform(-10, 10) for x in range(3)],
                color = [np.random.uniform(0.5, 1) for x in range(3)]
            )
...
class Engine:
    ...
    def __init__(self, scene):
        ...
        self.cube_mesh = ObjMesh("model/cube.obj")
        ...
        self.cubeTranformVBO = glGenBuffers(1)
        glBindBuffer(GL_ARRAY_BUFFER, self.cubeTranformVBO)
        ...
        #set up uniforms
        glUniformMatrix4fv(glGetUniformLocation(self.shaderTextured, "projection"), 1, GL_FALSE, projection_transform)

        glUniform3fv(glGetUniformLocation(self.shaderTextured, "ambient"), 1, np.array([0.1, 0.1, 0.1], dtype=np.float32))

        glUniform1i(glGetUniformLocation(self.shaderTextured, "material.albedo"), 0)
        glUniform1i(glGetUniformLocation(self.shaderTextured, "material.ao"), 0)
        glUniform1i(glGetUniformLocation(self.shaderTextured, "material.normal"), 0)
        glUniform1i(glGetUniformLocation(self.shaderTextured, "material.specular"), 0)
        ...
        #create asset
        self.light_mesh = UntexturedCubeMesh(l=0.1, w=0.1, h=0.1)
    ...
    def createShader(self, vertexFilepath, fragmentFilepath):
        ...
        shader = compileProgram(temp1, temp2)
    ...
    def draw(self, scene):
        ...
        view_transform = pyrr.matrix44.create_look_at(
            ...
            target= scene.player.position + scene.player.get_forward(),
            up= scene.player.get_up(),
            dtype=np.float32
        )

        glUseProgram(self.shaderTextured)
        glUniformMatrix4fv(self.viewLocTextured, 1, GL_FALSE, view_transform)
    ...
    def quit(self):
        ...
        glDeleteBuffers(1, (self.cubeTranformVBO, ))
