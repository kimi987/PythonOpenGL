...
class GraphicsEngine:
    ...
    def create_framebuffers(self):
        ...
        for i in range(2):
            ...
            new_color_buff_0 = glGenTextures(1)

            glBindTexture(GL_TEXTURE_2D, new_color_buff_0)
            ...
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, new_color_buff_0, 0)

            new_color_buff_1 = glGenTextures(1)
            glBindTexture(GL_TEXTURE_2D, new_color_buff_1)
            ...
            glBindTexture(GL_TEXTURE_2D, 0)
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, new_color_buff_1, 0)
            self.colorBuffers.append([new_color_buff_0, new_color_buff_1])
            ...
            glBindRenderbuffer(GL_RENDERBUFFER, self.depthStencilBuffers[i])
            glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, self.depthStencilBuffers[i])
    ...
    def setup_shaders(self):
        ...
        glUniformMatrix4fv(glGetUniformLocation(self.lighting_shader, "projection"), 1, GL_FALSE, projection_transform)
        ...
        glUniformMatrix4fv(glGetUniformLocation(self.unlit_shader, "projection"), 1, GL_FALSE, projection_transform)
        ...
        self.crt_shader = self.createShader("shaders/simple_post_vertex.txt", "shaders/post_fragment.txt")
    ...
    def create_assets(self):
        ...
        self.cube_mesh = Mesh("../Assets/models/cube.obj")
        self.medkit_texture = AdvancedMaterial("medkit")
        self.medkit_billboard = BillBoard(w = 0.6, h = 0.5)
        self.container_mesh = Mesh("../Assets/models/container.obj")
        self.monkey_mesh = Mesh("../Assets/models/monkey.obj")

        self.light_texture = Material("../Assets/gfx/greenlight.png")
...
class AdvancedMaterial:
    ...
    def __init__(self, fileroot):
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
        with Image.open(f"../Assets/gfx/{fileroot}_albedo.png", mode = "r") as img:
            image_width,image_height = img.size
            img = img.convert("RGBA")
            img_data = bytes(img.tobytes())
            glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,image_width,image_height,0,GL_RGBA,GL_UNSIGNED_BYTE,img_data)
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
        with Image.open(f"../Assets/gfx/{fileroot}_ao.png", mode = "r") as img:
            image_width,image_height = img.size
            img = img.convert("RGBA")
            img_data = bytes(img.tobytes())
            glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,image_width,image_height,0,GL_RGBA,GL_UNSIGNED_BYTE,img_data)
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
        with Image.open(f"../Assets/gfx/{fileroot}_glossmap.png", mode = "r") as img:
            image_width,image_height = img.size
            img = img.convert("RGBA")
            img_data = bytes(img.tobytes())
            glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,image_width,image_height,0,GL_RGBA,GL_UNSIGNED_BYTE,img_data)
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
        with Image.open(f"../Assets/gfx/{fileroot}_normal.png", mode = "r") as img:
            image_width,image_height = img.size
            img = img.convert("RGBA")
            img_data = bytes(img.tobytes())
            glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,image_width,image_height,0,GL_RGBA,GL_UNSIGNED_BYTE,img_data)
...
class Font:
    ...
    def __init__(self):
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR)
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
        with Image.open("../Assets/gfx/Inconsolata.png", mode = "r") as img:
            image_width,image_height = img.size
            img = img.convert("RGBA")
            img_data = bytes(img.tobytes())
            glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,image_width,image_height,0,GL_RGBA,GL_UNSIGNED_BYTE,img_data)
